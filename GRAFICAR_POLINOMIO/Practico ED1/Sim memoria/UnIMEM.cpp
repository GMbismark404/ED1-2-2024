...
void CSmemoria::Imprimirrango(TColor FormColor, TCanvas *Canvas,int posX,int posY,int A , int B){
   if (A<=B) {
	int auxX = posX;
	int auxY = posY;
		Canvas->Font->Size =7;
	...
	int p=0;
	   int y = 30;int rowHeight = 20;  Canvas->Font->Size =8;
	Pintado(posX,posY-p,"Direccion",FormColor,Canvas);
	posX+=TamanoCeldaX;
	Pintado(posX,posY-p,"Dato",FormColor,Canvas);
	posX+=TamanoCeldaX;
	Pintado(posX,posY-p,"ID",FormColor,Canvas);
	...
	MessageDlg("A tiene que ser MAYOR que B", mtWarning, TMsgDlgButtons() << mbOK, 0);
   }
   }
