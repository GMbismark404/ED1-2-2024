...
direccion PolinomioS::BuscarTerminoN(int i) {   //busca el término de un polinomio dado una posición
	direccion dir = this->ptr_Poli;
	if (dir != NULO) {
...
int PolinomioS::exponente(int nroter) { // dado la posición del termino obtienes el exp
	direccion dir = BuscarTerminoN(nroter); //Busca el termino dado la posición
	if (dir != NULO) {
